...
mp4 rendered with classic MuJoCo from the recorded qpos trajectory.
    ...
    python legacy_trainer_to_port/evaluate.py --run legacy_trainer_to_port/runs/v2_s1_planar_s0 --episodes 16
    python legacy_trainer_to_port/evaluate.py --run legacy_trainer_to_port/runs/v2_s1_planar_s0 --video dash.mp4 --seconds 40
    python legacy_trainer_to_port/evaluate.py --run ... --dr            # with the training randomization on
...
def load_run(run, checkpoint=None, n_envs=16, dr=False, free_clock=False,
              warm_start=False):
    run = Path(run)
    ...
    if free_clock:
        ...
        # event and the clock free-runs (controller/deploy/controller_v2.py says so in its docstring).
...
def rollout(env, agent, seed, n_max, record=False, stoplight=0.0):
    ...
    params = EnvParams.final(agent.cfg)._replace(dr_scale=1.0 if env.cfg.dr_enable else 0.0,
                                                 ctrl_jitter_ms=0.0, ctrl_drop_prob=0.0,
