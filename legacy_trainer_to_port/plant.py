...
class Plant:
    def __init__(self, cfg):
        ...
        # the 6-DOF base spring (walk_v4): the effective mass or inertia each base DOF presents at the
        # keyframe -- the joint-space mass matrix diagonal -- so the damping can be CRITICAL at every
        # stiffness, c = 2 sqrt(k M). Only the DOFs this model has (planar: x, z, pitch).
        _names = ("x", "y", "z", "roll", "pitch", "yaw")
        self.spring_sel = np.array([i for i, n in enumerate(_names) if self.base_d[n] >= 0])
        self.spring_qadr = np.array([self.base_q[_names[i]] for i in self.spring_sel])
        self.spring_dadr = np.array([self.base_d[_names[i]] for i in self.spring_sel])
        # M e_a picks column a; its a-th entry is the diagonal. mj_mulM, not mj_fullM: fullM's
        # signature changed in MuJoCo 3.13 (and d.qM became d.M), and the two venvs differ
        def _m_diag(a):
            e, out = np.zeros(m.nv), np.zeros(m.nv)
            e[a] = 1.0
            mujoco.mj_mulM(m, d, out, e)
            return out[a]
        self.spring_M = np.array([_m_diag(a) for a in self.spring_dadr])
