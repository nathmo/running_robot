"""walk_v4 smoke test: the invariants the artifact makes checkable, run before any GPU hour.
    ...
    python legacy_trainer_to_port/smoke_test.py            # everything (a few minutes on CPU)
    python legacy_trainer_to_port/smoke_test.py --quick    # no env stepping (seconds)
...
def test_v4():
    """v4 = v3 without the reflexes -- now including their three latched dims, so the action is 47
    wide and the *_reflex control twins are gone. Checked on every v4 preset: the per-joint residual
    authority is what the preset asked for, and the run/stop twins differ only in the command."""
    print("v4: reflex-free presets")
    from dataclasses import asdict
    rng = np.random.default_rng(1)
    nominal = np.array([0, 0, 0.12, 0, 0, -0.12])
    names = [n for n in get_config.__globals__["PRESETS"] if n.startswith("v4_") or n == "v4"]
    check("v4 presets exist (joystick 3 + run/stop 2 + nolane + one-run x 2)", len(names) == 8, str(names))
    # the base spring's law on a 1-DOF body: critically damped at EVERY stiffness means no overshoot
    # from a released deflection, however soft the curriculum has made it
    from env import spring_law, spring_anchor
    M, w0 = 3.0, 2.0 * np.pi * 2.0
    k0, c0 = jnp.array([M * w0 ** 2]), jnp.array([2.0 * M * w0])
    for s in (1.0, 0.1, 0.01):
        x, v, lo_x = 0.1, 0.0, 0.1
        for _ in range(int(8.0 / (w0 * np.sqrt(s)) / 1e-3)):          # ~8 time constants at 1 kHz
            F = float(spring_law(jnp.array([x]), jnp.array([v]), s, k0, c0, jnp.array([1.0]))[0])
            v += F / M * 1e-3
            x += v * 1e-3
            lo_x = min(lo_x, x)
        check(f"base spring s={s:g}: critically damped (settles, no overshoot)",
              lo_x > -1e-3 * 0.1 and abs(x) < 0.01 * 0.1, f"end {x:+.2e}, min {lo_x:+.2e}")
    z = spring_law(jnp.array([0.1]), jnp.array([0.2]), 0.0, k0, c0, jnp.array([1.0]))
    zm = spring_law(jnp.array([0.1]), jnp.array([0.2]), 1.0, k0, c0, jnp.array([0.0]))
    check("base spring: s = 0 or masked -> no force", float(jnp.abs(z).max()) == 0.0 and float(jnp.abs(zm).max()) == 0.0)
    a = spring_anchor(jnp.array([0.0]), jnp.array([1.0]), jnp.array([0.0]), 0.01, jnp.array([0.05]))
    check("base spring leash: an anchor 1 m behind is dragged to 5 cm", abs(float(a[0]) - 0.95) < 1e-6, f"{float(a[0]):.3f}")
    ...
    for name in names:
        cfg = get_config(name)
        p = gait.GaitParams.from_cfg(cfg)
        control = name.endswith("_reflex")
        if name.startswith("v4_one"):
            flat = [k for g in cfg.curriculum_order for k in ((g,) if isinstance(g, str) else g)]
            check(f"{name}: one run -- spring on, no lane, spring queued where the pitch wheel was",
                  cfg.base_spring_enable and cfg.w_lane == 0.0 and "base_spring" in flat
                  and cfg.model_path.endswith("free.xml"), f"order {flat}")
            check(f"{name}: heading billed on the averaged estimate; the compute of the three stages",
                  cfg.heading_avg_s > 0 and cfg.heading_euler and cfg.total_steps == 450_000_000,
                  f"tau {cfg.heading_avg_s} s, {cfg.total_steps / 1e6:.0f} M")
            # the spring must be OFF before its curriculum group runs out of turn, or the run ends
            # leaning on it: worst case is the forced decay starting at the deadline and running full
            # the spring must be able to reach zero inside its group's cap even if every rollout
            # advances: past the cap the queue hands over and the run trains on a crutch it keeps
            check(f"{name}: the spring can release inside its curriculum group's cap",
                  0 < cfg.base_spring_decay_steps <= cfg.curriculum_group_max_steps,
                  f"{cfg.base_spring_decay_steps / 1e6:.0f} M of {cfg.curriculum_group_max_steps / 1e6:.0f} M")
        if name == "v4_nolane":
            # the lane test must change the lane weight and nothing else, or v4_s* stops being its control
            d = sorted(k for (k, v), w in zip(asdict(cfg).items(), asdict(get_config("v4")).values()) if v != w)
            check("v4_nolane: = v4 with w_lane 0 and nothing else", d == ["w_lane"] and cfg.w_lane == 0.0, str(d))
        if "_runstop" in name:
            # a run/stop preset is its joystick twin with the command and the RUN income changed, and
            # nothing else -- so the joystick results stay a valid control for it
            twin = get_config(name.replace("_runstop", ""))
            d = sorted(k for (k, v), w in zip(asdict(cfg).items(), asdict(twin).values()) if v != w)
            # v_max appears here only when the twin's v_max differs from v_ceiling (run/stop pins
            # them equal); v4_one already sets v_max = v_ceiling, so it drops out of the diff
            check(f"{name}: = its joystick twin + binary command + linear RUN income",
                  set(d) <= {"cmd_binary", "eval_ladder", "run_income_linear", "v_max"}
                  and {"cmd_binary", "eval_ladder", "run_income_linear"} <= set(d), str(d))
            check(f"{name}: full stick = the income cap", cfg.v_max == cfg.v_ceiling,
                  f"v_max {cfg.v_max} v_ceiling {cfg.v_ceiling}")
    ...
    # a scalar residual_scale (every config before v4) still broadcasts
    old = get_config("v2_s2_free")
    p2 = gait.GaitParams.from_cfg(old)
    check("scalar residual_scale broadcasts to 6",
          isinstance(old.residual_scale, float) and p2.residual_scale == (old.residual_scale,) * 6,
...
def test_spring_env(quick):
    """The base spring inside the env: on at the start of a v4_one run, off at the final point, billed
    only when on, and -- the positive control -- it actually holds the body up."""
    if quick:
        return
    print("v4: the 6-DOF base spring in the env")
    from env import DashEnvV2, EnvParams
    from ppo import initial_params
    cfg = get_config("v4_one")
    env = DashEnvV2(cfg, n_envs=4)
    p0 = initial_params(cfg)
    check("initial_params: spring at full stiffness", p0.base_spring == 1.0, f"{p0.base_spring}")
    check("EnvParams.final: spring off (evals and the robot never see it)", EnvParams.final(cfg).base_spring == 0.0)
    pl = env.plant
    iq = [pl.base_q[n] for n in ("roll", "pitch")]
    res = {}
    for s in (1.0, 0.0):
        prm = p0._replace(base_spring=s, dr_scale=0.0)
        st, _ = env.reset(jax.random.PRNGKey(3), prm)
        st = st.replace(spring_mask=jnp.ones_like(st.spring_mask))    # every DOF on for the comparison
        a = jnp.zeros((4, env.action_dim))
        tilt, bill, w = 0.0, [], []
        for _ in range(60):
            st, _, _, done, info = env.step(st, a, prm)
            tilt = max(tilt, float(jnp.abs(st.data.qpos[:, iq]).max()))
            bill.append(float(jnp.mean(info["reward_terms"]["base_spring"])))
            w.append(float(jnp.mean(info["spring_w"])))
        res[s] = (tilt, min(bill), max(bill), min(w))
    check("spring on: billed, and every episode has an assisted DOF", res[1.0][1] < 0.0 and res[1.0][3] == 1.0,
          f"bill {res[1.0][1]:+.3f}")
    check("spring off: billed exactly nothing", res[0.0][1] == 0.0 == res[0.0][2])
    check("spring on holds the base: max roll/pitch smaller than off (positive control)",
          res[1.0][0] < res[0.0][0], f"{np.degrees(res[1.0][0]):.1f} vs {np.degrees(res[0.0][0]):.1f} deg")
    # heading bills the averaged ESTIMATE (the wiring; the averaging itself is unit-tested in test_v4):
    # hold the robot's own heading at 0.3 rad and read the term back every tick
...
def test_env(quick):
    ...
    env = DashEnvV2(cfg, n_envs=4)
    check("policy variant: actor 384 / obs 409 / action 47", (env.actor_dim, env.obs_dim, env.action_dim) == (384, 409, 47),
