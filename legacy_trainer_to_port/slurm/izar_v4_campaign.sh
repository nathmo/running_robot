...
# from THAT seed's best.msgpack (never final -- see legacy_trainer_to_port/README.md). Stage 3 is submitted by
# hand after tools/compare_runs.py has ranked the stage-2 seeds.
#
#   legacy_trainer_to_port/slurm/izar_v4_campaign.sh            # from the repo root on the login node
#
...
for arm in "" _reflex; do
    for s in 0 1 2; do
        ...
        j2=$(sbatch --parsable --gres=gpu:2 --dependency=afterok:"$j1" \
             --export=ALL,PRESET=v4${arm},NAME=$n2,SEED=$s,$COMMON,WARM=$HOME/running_robot/legacy_trainer_to_port/runs/$n1/best.msgpack \
             legacy_trainer_to_port/slurm/izar_train.sbatch)
