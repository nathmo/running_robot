...
distribution, and so is the release transient (the legs take the full weight in one tick).
    ...
    python legacy_trainer_to_port/tools/bringup_probe.py --run legacy_trainer_to_port/runs/v2c_s2_free_dp2x_s0 --sweep basic
    python legacy_trainer_to_port/tools/bringup_probe.py --run ... --hold 3.0 --pitch 10 --video drop.mp4
...
def load(run, checkpoint=None, n_envs=16, dr=False):
    run = Path(run)
