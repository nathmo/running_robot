"""Compare two v2 traces (legacy_trainer_to_port/tools/trace.py format) from two implementations.

    python legacy_trainer_to_port/tools/compare_traces.py trace_mjx.json trace_cpu.json

Reports:
  1. FUNCTIONAL agreement on shared states: for every tick of trace B, recompute the gait
     targets / kp / kd from B's own (spec, phase, state) with legacy_trainer_to_port/gait.py in numpy and diff
...
def recompute(tr, row, prev_row, prev_target):
    """The v4 control law on the recorded state: gait.assemble, joint clip, no-load slew limit.

    No reflex state to carry any more -- v4 deleted both reflexes, so the target is the latched
    spec's feedforward plus the residual and nothing else."""
    gpd = {k: (tuple(v) if isinstance(v, list) else v) for k, v in tr["gait_params"].items()}
    gpd.pop("pitch_reflex_rate_lp", None)          # older traces still carry it
    ...
    target, kp, kd, _ = gait.assemble(spec, res, phi, np.array(tr["nominal_ctrl"]), gp, xp=np)
...
def self_consistency(tr, name):
    ...
    for row in tr["rows"]:
        ...
        target, kp, kd = recompute(tr, row, prev, prev_target)
