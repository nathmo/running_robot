...
training configurations could not answer (legacy_trainer_to_port/README.md). Before spending another training
...
parameters, so the CEM converges fast.
    ...
    python legacy_trainer_to_port/tools/speed_lib.py --run ... --checkpoint ... --targets 1.0,1.5,2.0,2.5,3.0
...
def main():
    ...
    params = EnvParams.final(cfg)._replace(dr_scale=0.0, ctrl_jitter_ms=0.0, ctrl_drop_prob=0.0,
                                           stoplight_prob=0.0, sprint_dist_m=1e4)
    dt = env.control_dt
