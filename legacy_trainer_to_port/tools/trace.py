"""Cross-implementation trace: the numbers two implementations of the v2 design must agree on.
    ...
    python legacy_trainer_to_port/tools/trace.py --out legacy_trainer_to_port/results/trace_mjx.json [--preset v2_s2_free] [--ticks 300]
    python legacy_trainer_to_port/tools/trace.py --rig --out legacy_trainer_to_port/results/trace_mjx_rig.json      # welded in the air
    python legacy_trainer_to_port/tools/compare_traces.py legacy_trainer_to_port/results/trace_mjx.json <other_impl_trace.json>
...
Protocol (so the CPU/walk_mit arm can produce the same file):
  * plant: nominal (dr off, noise off, no pushes/wind/trips, thermal cold), delay 12 ms, reset at
    the keyframe with NO joint noise. There is no base assist to hold an open-loop gait up any more
    (v4 deleted the wheels; the 6-DOF spring is a curriculum, off at EnvParams.final), so on the
    ground the trace is short by nature -- use --rig for a long one
  ...
  * per tick, recorded AFTER the tick: t, phase, commit flag, spec (41), target (6), kp (6), kd (6),
  ...
  * --rig: the in-air test rig (walk_mit set_fixed_base): the base is re-pinned at the keyframe
    ...
    gait cycles for the whole trace and the physics stays close between implementations. On the
...
trace's state via legacy_trainer_to_port/gait.py in numpy) and the trajectory divergence time.
"""
import argparse
import json
import sys
...
FIXED_SPEC = np.zeros(gait.SPEC_DIM)      # 41 since v4 deleted the roll-reflex dims
FIXED_SPEC[1] = 0.3        # cam a1
FIXED_SPEC[8] = -0.2       # thigh a1
FIXED_SPEC[gait.I_FREQ] = -0.2      # 2.3 Hz
...
def main():
    ...
    params = EnvParams.final(cfg)._replace(dr_scale=0.0, ctrl_jitter_ms=0.0,
                                           ctrl_drop_prob=0.0)
