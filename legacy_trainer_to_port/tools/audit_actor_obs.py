...
perturbation did not bite, which the harness checks too).
    ...
    python legacy_trainer_to_port/tools/audit_actor_obs.py --run legacy_trainer_to_port/runs/<run> [--checkpoint ...]
...
def _is_information(i, actor_dim, frame_dim=FRAME_DIM):
...
def _where(i, actor_dim, frame_dim=FRAME_DIM):
