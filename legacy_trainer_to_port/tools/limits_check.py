...
node: x_{k+1} = x_k + (dt/tau_th)(tau_rms^2/(tau_cont^2 s) - x_k).
    ...
    python legacy_trainer_to_port/tools/limits_check.py --run legacy_trainer_to_port/runs/v2c_s2_free_dp2x_s0
    python legacy_trainer_to_port/tools/limits_check.py --run ... --checkpoint ...best_88473600.msgpack --dr
    python legacy_trainer_to_port/tools/limits_check.py --run ... --episodes 8 --seconds 45 --json out.json
"""
import argparse
import json
import sys
...
# --------------------------------------------------------------------------- rollout
def record(env, agent, seed, n_max, dr_scale=0.0):
...
def main():
    ...
    rec = record(env, agent, args.seed, n_max, dr_scale=0.0)
    ...
    if args.dr:
        ...
        rec_d = record(env_d, agent_d, args.seed, n_max, dr_scale=1.0)
    ...
    if args.dr_sweep:
        ...
        for lv in [float(x) for x in args.dr_sweep.split(",")]:
            r = record(env_s, agent_s, args.seed, n_max, dr_scale=lv)
            liv = r["alive"] > 0.5
