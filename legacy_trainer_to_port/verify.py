"""Does this policy meet the contract? One command, one PASS/FAIL table, no interpretation needed.
    ...
    python legacy_trainer_to_port/verify.py --run legacy_trainer_to_port/runs/v3_s0
    python legacy_trainer_to_port/verify.py --run legacy_trainer_to_port/runs/v3_s0 --checkpoint legacy_trainer_to_port/runs/v3_s0/best.msgpack
...
def check_curriculum(rep, run, ckpt, cfg):
    ...
    if getattr(cfg, "base_spring_enable", False):
        rep.add("1. trained as advertised", "6-DOF training spring fully released",
                float(ep.get("base_spring", 1.0)) == 0.0, f"{ep.get('base_spring', 1.0):.3f}", "= 0.000",
                "a policy still leaning on the spring has never stood on its own")
    return d
...
def ladder_eval(cfg, agent, n_envs, seconds, dr, bringup, seed, quiet=True, greedy=True,
    ...
    if built is None:
        ...
        params = EnvParams.final(cfg)._replace(
            dr_scale=1.0 if dr else 0.0, ctrl_jitter_ms=0.0, ctrl_drop_prob=0.0, 
...
def check_privilege(rep, cfg, env, agent, args):
    ...
    # 20x, not 100x. `floor` is already the threshold a leak must stay under, and what the positive
    # control has to establish is that the probe can resolve a channel-sized effect COMFORTABLY
    # above that threshold -- an order of magnitude of headroom, not two. 100x was picked on the
    # planar plant and is unreachable on the free one by construction: the channel's full swing is
    # 1.0 and the free plant's re-run noise puts 100x floor at 1.87, so the test could only ever
    # SKIP. A 20x bar still means a leak twenty times smaller than the command channel is caught.
    MARGIN = 20.0
    if MARGIN * floor > 1.0:
        ...
        rep.add("6. no privilege", "POSITIVE CONTROL: the command does reach the actor",
                ctrl > MARGIN * floor, f"{ctrl:.2e}", f"> {MARGIN * floor:.2e}",
...
def check_deploy(rep, run, ckpt, env, agent, args):
    ...
    sys.path.insert(0, str(PKG.parent / "controller" / "deploy"))
    try:                                        # controller/deploy modules import each other flat
