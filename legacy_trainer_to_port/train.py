"""Train a DASH-01 Walker v2 policy: PPO in JAX over N MJX environments on one GPU.
    ...
    python legacy_trainer_to_port/train.py --preset v2_smoke --steps 4000 --n-envs 8
    # the run (one RTX PRO 6000 / B200; ~4096 envs)
    python legacy_trainer_to_port/train.py --preset v2_s1_planar --steps 300000000 --name v2_s1_planar_s0
    # S1 -> S2 warm start (identical obs/action widths, §11)
    python legacy_trainer_to_port/train.py --preset v2_s2_free --warm-start legacy_trainer_to_port/runs/v2_s1_planar_s0/final.msgpack
    # cluster requeue-safe: the same command is correct for the first start and every restart
    python legacy_trainer_to_port/train.py --preset v2_s2_free --name v2_s2_free_s0 --resume auto

Outputs -> legacy_trainer_to_port/runs/<name>/: ckpt_<steps>.msgpack (+ .json with the curriculum state),
...
def make_eval_env(cfg, n=16):
    """The greedy-eval plant: nominal, no noise, no disturbances, nominal delay.

    There is no crutch to keep: v4 has one base assist (the 6-DOF spring) and EnvParams.final turns
    it off, so an eval env is the deployed plant by construction."""
...
def run_eval(agent, cfg, eval_env):
    ...
    pt = EnvParams.final(cfg)._replace(dr_scale=0.0, ctrl_jitter_ms=0.0, ctrl_drop_prob=0.0,
                                       bringup_scale=0.0, bringup_p_drop=0.0,
                                       bringup_p_held=0.0)
    ...
    while agent.step < total:
        ...
        if agent.rollout_n == 1 or agent.rollout_n % 10 == 0:
            ...
            print(f"               dr {row.get('curriculum/dr_scale', 0):.3f} "
                  ...
                  + (f" spring {row.get('curriculum/base_spring', 0):.3f} "
                     f"(defl {row.get('curriculum/base_spring_defl', float('nan')):.2f})"
                     if getattr(cfg, "base_spring_enable", False) else ""), flush=True)
