...
from evaluate import build  # noqa

ap = argparse.ArgumentParser()
ap.add_argument("run"); ap.add_argument("ckpt")
...
def snap():
    ...
    cause_buf.append(dict(low=z < cfg.term_height, tip=raw._gravity_body()[2] > cfg.term_gravity_z,
                          floor=bool(raw._floor_violation()), ws=bool(raw._workspace_violation()),
                          nan=not np.all(np.isfinite(d.qpos)), z=z, pitch=float(d.qpos[4])))
...
for e in range(args.episodes):
    ...
    while True:
        ...
        a0 = np.clip(a[0], -1, 1)
        res.append(a0[44:50]); n += 1
        v2 = info[0].get("v2", {})
        if v2.get("commit"):
            ncommit += 1; fhz.append(v2["f_hz"])
        ...
        td.append(v2.get("td_err", np.nan)); theta = max(theta, v2.get("theta_max", 0.0))
        ...
        if d[0]:
            ...
            break
    res = np.asarray(res); dev = np.asarray(dev)
...
T = np.array([r["t"] for r in rows]); D = np.array([r["dist"] for r in rows])
