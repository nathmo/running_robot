...
class Config:
    ...
    # ----- PER-STEP IMPEDANCE CHANNEL (2026-08-26; MIT-mode force control, walk_mit only) -------
    # imp_enable appends 4 action dims [kp_L, kd_L, kp_R, kd_R]: per-LEG multipliers on the servo
    # gains, exp-mapped so neutral 0 -> 1.0, +1 -> *_up, -1 -> 1/*_dn. In MIT mode the drive takes
    # Kp/Kd per FRAME (robot/MIT_PROTOCOL.md), so a per-step channel at 200 Hz is the deployable
    # interface; the policy's phase input makes it phase-dependent impedance for free. Headroom
    # mirrors the MEASURED frame ranges: Kp field 0-500 N*m/rad (plant kp 200 -> up 2.5x), Kd
    # field 0-5 N*m*s/rad (plant kv 5 sits AT the ceiling -> up 1.0, soften-only). Billed by
    # w_imp_rate as a per-step channel (like the residual), NOT by the phase-gated coef_rate.
    imp_enable: bool = False
    imp_kp_up: float = 2.5
    imp_kp_dn: float = 3.0
    imp_kd_up: float = 1.0
    imp_kd_dn: float = 4.0
...
PRESETS.update({
    ...
    "walk_fwd_m4_mit": lambda: _walk_fwd3(base_lock=LOCKS["m4"], drive_bandwidth_hz=12.0,
                                          ...
                                          cmd_v_fwd_start=0.50, cmd_v_back_start=0.15,
                                          **_LADDER_RWD),
    # FALLBACK ARMS (walk_mit only, 2026-08-26 overnight run). The deliverable is A WALKING POLICY;
    # realism is negotiable, in this order and no further (user's call: a policy for a robot we
    # don't have beats no policy for the robot we have). Same relaxations as walk_fwd_easy, the
    # proven diagnostic arm: no plant DR (perfect calibration), no sensor noise (perfect IMU/homing),
    # no pushes/trips, no control-timing jitter. The MIT drive numbers stay -- they are the ONE
    # constraint this study exists to test. Launch only per walk_mit/MONITOR.md rules.
    "walk_fwd_m2_mit_easy": lambda: _walk_fwd3(base_lock=LOCKS["m2"], drive_bandwidth_hz=12.0,
                                               drive_delay_ms=7.0, ankle_mode="rigid",
                                               cmd_v_fwd_start=0.50, cmd_v_back_start=0.15,
                                               dr_enable=False, obs_noise_enable=False,
                                               push_interval_s=0.0, trip_prob=0.0,
                                               ctrl_jitter_ms_final=0.0, ctrl_drop_prob_final=0.0,
                                               **_LADDER_RWD),
    "walk_fwd_m3_mit_easy": lambda: _walk_fwd3(base_lock=LOCKS["m3"], drive_bandwidth_hz=12.0,
                                               drive_delay_ms=7.0, ankle_mode="rigid",
                                               cmd_v_fwd_start=0.50, cmd_v_back_start=0.15,
                                               dr_enable=False, obs_noise_enable=False,
                                               push_interval_s=0.0, trip_prob=0.0,
                                               ctrl_jitter_ms_final=0.0, ctrl_drop_prob_final=0.0,
                                               **_LADDER_RWD),
    # THE COLD IMPEDANCE ARMS (2026-08-26, user's call): from scratch with +4 per-leg kp/kd action
    # dims (imp_enable). Cold because the action width changes 26 -> 30 (and prev_action widens the
    # obs frame with it) -- no existing checkpoint fits. m3 warm-starts from imp m2 WITHIN this
    # lineage; the plain _mit warm runs stay up as the fixed-gain control arm of the A/B.
    "walk_fwd_m2_mit_imp": lambda: _walk_fwd3(base_lock=LOCKS["m2"], drive_bandwidth_hz=12.0,
                                              drive_delay_ms=7.0, ankle_mode="rigid",
                                              cmd_v_fwd_start=0.50, cmd_v_back_start=0.15,
                                              imp_enable=True,
                                              **_LADDER_RWD),
    "walk_fwd_m3_mit_imp": lambda: _walk_fwd3(base_lock=LOCKS["m3"], drive_bandwidth_hz=12.0,
                                              drive_delay_ms=7.0, ankle_mode="rigid",
                                              cmd_v_fwd_start=0.50, cmd_v_back_start=0.15,
                                              imp_enable=True,
                                              **_LADDER_RWD),
    # eval diagnostic only (2026-08-27): the imp arm with ALL base DoF free, for "what does the
    # m2-trained policy do unconstrained" renders. Not a training arm tonight.
    "walk_fwd_m6_mit_imp": lambda: _walk_fwd3(base_lock=LOCKS["m6"], drive_bandwidth_hz=12.0,
                                              drive_delay_ms=7.0, ankle_mode="rigid",
                                              cmd_v_fwd_start=0.50, cmd_v_back_start=0.15,
                                              imp_enable=True,
                                              **_LADDER_RWD),
    "walk_fwd_m2_mit_imp_easy": lambda: _walk_fwd3(base_lock=LOCKS["m2"], drive_bandwidth_hz=12.0,
                                                   drive_delay_ms=7.0, ankle_mode="rigid",
                                                   cmd_v_fwd_start=0.50, cmd_v_back_start=0.15,
                                                   imp_enable=True,
                                                   dr_enable=False, obs_noise_enable=False,
                                                   ...
                                                   **_LADDER_RWD),
    "walk_fwd_m3_mit_imp_easy": lambda: _walk_fwd3(base_lock=LOCKS["m3"], drive_bandwidth_hz=12.0,
                                                   drive_delay_ms=7.0, ankle_mode="rigid",
                                                   cmd_v_fwd_start=0.50, cmd_v_back_start=0.15,
                                                   imp_enable=True,
                                                   dr_enable=False, obs_noise_enable=False,
