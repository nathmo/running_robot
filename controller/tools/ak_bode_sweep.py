...
v_des and tau_ff stay 0 -- controller/deploy/mit.py raises on anything else -- so no unidentified
...
Usage (on the Pi, one actuator x one mode per invocation):
    python controller/tools/ak_bode_sweep.py --channel can1 --id 106 --mode servo --out /tmp/x.npz
    python controller/tools/ak_bode_sweep.py --channel can1 --id 106 --mode mit   --out /tmp/y.npz
