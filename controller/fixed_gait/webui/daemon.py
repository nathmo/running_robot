...
import thermal_excite                     # controller/deploy/ — the burst law + its envelope
# controller/deploy/, all three pure (numpy / struct / json) and importing nothing from the webui: the
...
# thigh +-1.047 rad, abduction +-0.785 rad — mujoco/RLframework/build_model.py J dict).
...
#
# The approach numbers come straight from controller/deploy/run_policy.py, which is the version of this
...
class RobotDaemon(threading.Thread):
    ...
    # -----------------------------------------
    # controller/deploy/run_policy.py does exactly this from a terminal, and it refuses to start while
    ...
    def policy_arm(self, spec):
        ...
        try:
            ...
            return False, ("controller/deploy/deploy_map.json does not exist -- the model->motor joint "
                           "map has never been built on this robot. Run "
                           "controller/deploy/make_deploy_map.py after verifying fklut."), info
