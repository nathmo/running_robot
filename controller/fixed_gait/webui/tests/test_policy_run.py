...
WHY THIS EXISTS
---------------
Until 2026-09-01 a real policy run meant stopping the web UI and running controller/deploy/run_policy.py
...
true for that to be safe rather than merely convenient:
  ...
  * letting go must stop the robot. The dead-man is the one guard whose absence is invisible.
    ...
    python -m pytest controller/fixed_gait/webui/tests/test_policy_run.py -v
"""
import atexit
import json
import os
...
# random weights (controller/deploy/tests/v2_fixture.py) -- because what is under test is the plumbing:
...
needs_v2 = pytest.mark.skipif(v2_fixture is None, reason="controller/deploy/tests/v2_fixture.py absent")
