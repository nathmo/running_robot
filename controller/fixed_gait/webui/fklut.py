...
fk_lut.npz is generated OFFLINE on the desktop by mujoco/RLframework/gen_fk_lut.py from the real
    ...
    def __init__(self, lut_path=paths.FK_LUT_FILE, map_path=paths.MODEL_MAP_FILE):
        ...
        if not self.try_reload():
            print(f"(no FK LUT at {lut_path} — EE animation disabled; generate it with "
                  f"mujoco/RLframework/gen_fk_lut.py — it hot-loads once the file appears)")
