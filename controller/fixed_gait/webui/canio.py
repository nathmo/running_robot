...
# controller/deploy/mit.py -- that module is the single definition of the wire format and it is written
...
# NOT the classic-MIT standard-id protocol that every mit_*.py in controller/tools implements. See the
...
def _mit_decode(buf):
    ...
    is controller/deploy/mit.py; imported from there when it is reachable so there is ONE definition,
        ...
        with self._lock:
            ...
            if cmd == CAN_PACKET_SET_POS:
                ...
                m.target = (struct.unpack(">i", bytes(msg.data[:4]))[0] / 10_000.0
                            - m.origin_shift)
                m.limp = False
            elif cmd == 8:                       # force control -- see controller/deploy/mit.py
