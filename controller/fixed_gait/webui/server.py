...
# exactly one owner, so controller/deploy/run_policy.py (which does the same job from a terminal)
...
def _policy_path(fname):
    ...
    go through _policy_read_path, which also sees controller/deploy/bundles/."""
...
def _load_bundle(path):
    from bundle import Bundle          # controller/deploy — numpy+json only, importable on the Pi
...
def _policy_preflight(bundle_path=None):
    ...
    if os.path.exists(jm_path):
        ...
        jm_ok, jm_why = False, ("controller/deploy/deploy_map.json does not exist — the model→motor "
