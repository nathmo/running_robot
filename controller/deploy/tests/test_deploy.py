...
for p in (str(DEPLOY), str(REPO / "controller" / "fixed_gait" / "webui")):
...
def test_vendored_gait_is_identical_to_the_training_module():
    """controller/deploy/fourier_gait.py is a byte-for-byte copy of walk_mit/fourier_gait.py. If they drift, the
    ...
    assert h[0] == h[1], ("controller/deploy/fourier_gait.py has drifted from walk_mit/fourier_gait.py -- "
...
class TestThermalStore:
    def store(self, tmp_path):
        sys.path.insert(0, str(REPO / "controller" / "fixed_gait" / "webui"))
        import thermalstore
