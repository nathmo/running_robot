# `controller/deploy` — running a trained policy on DASH-01
...
|---|---|---|
| exporter | `controller/deploy/export_policy.py` | `walk_v2/export.py` |
...
```bash
python controller/deploy/run_policy.py --bundle .../v2_run.npz \
    --jointmap controller/deploy/deploy_map.json --command-file /tmp/dash_command --max-seconds 30
...
```bash
python controller/deploy/run_policy.py --bundle .../v3_joy.npz \
    --jointmap controller/deploy/deploy_map.json --command-file /tmp/dash_command --max-seconds 30
...
`controller/deploy/tests/test_v2_deploy.py` drives `PolicyControllerV2` through those recorded states
...
python walk_v2/export.py --run walk_v2/runs/v2c_s2_free_dp2x_stop_s0 \
    --out controller/deploy/bundles/v2c_stop_s0.npz
python -m pytest controller/deploy/tests -q
python controller/deploy/run_policy.py --bundle controller/deploy/bundles/v2c_stop_s0.npz --mock \
...
```bash
python controller/deploy/export_policy.py --run walk_mit/runs/imp_m2_long \
    --out controller/deploy/bundles/imp_m2_long_204M.npz
python controller/deploy/verify_export.py --bundle controller/deploy/bundles/imp_m2_long_204M.npz \
...
```bash
python controller/deploy/thermal_fit.py --campaign controller/fixed_gait/webui/data/thermal_runs.json \
    --motor left.thigh --motor-type AKE90-8 --motor-mass 0.85 --r-phase 0.10 \
    --out controller/deploy/thermal_params.json
```
...
```bash
python controller/deploy/thermal_calibrate.py --motor left.thigh --mock --steps 6:20,0:60   # rehearse
sudo systemctl stop runningrobot-webui.service
python controller/deploy/thermal_calibrate.py --motor left.thigh --joint-is-blocked \
    --steps 7:4000,0:4600
python controller/deploy/thermal_fit.py --logs .../thermal_left_thigh_*.npz \
...
```bash
python controller/deploy/run_policy.py --bundle .../imp_m2_long_204M.npz --mock --no-imu \
...
```bash
sudo systemctl stop runningrobot-webui.service
python controller/deploy/run_policy.py --bundle .../imp_m2_long_204M.npz \
    --jointmap controller/deploy/deploy_map.json --thermal controller/deploy/thermal_params.json \
...
an extended-id VESC-style command 8, byte order **kp first**, and every `mit_*.py` in `controller/tools`
