...
def initial_params(c):
    ...
    base_spring=1.0 if getattr(c, 'base_spring_enable', False) else 0.0)
...
class PPO:
        ...
        def rollout(params, stats, env_state, obs, key, env_params):
            def one(carry, _):
                ...
                metrics = dict(
                    ...
                    spring_defl=info["spring_defl"], spring_w=info["spring_w"],
    ...
    def _spring_gate(self, defl, d_steps, ep_len, gate, retreat_frac):
        """The 6-DOF base spring's stiffness (walk_v4), released on the ROBOT'S PERFORMANCE at the
        stiffness it currently has -- the same competence signal, bars and hysteresis as every other
        curriculum (`_gated`), and monotone for the same reason.

        Gating on DEFLECTION instead (softening while e = F_needed/(s k0) stays small) looks like the
        physically meaningful signal and is a trap: softening the spring RAISES the deflection a given
        lean produces, so the loop settles at whatever stiffness holds deflection at the threshold and
        stops there. Measured on the first v4_one seeds -- 1.00 -> 0.80 in 46 M live steps, 5% of the
        log distance, i.e. ~900 M to release against an 80 M group cap. Patching that with a timer
        ("decay on schedule after 15 M regardless") is a schedule wearing a gate's clothes.

        What the run actually needs to know is whether the policy can hold itself up as the help goes
        away, and episode length says exactly that. Deflection keeps its one honest job: if the robot
        is SAGGING into the spring (defl > base_spring_retreat) the stiffness goes back up, whatever
        the episode lengths say, because that is the policy hanging on the crutch rather than using
        it. Log-space: each halving is one step for the policy. Below base_spring_min it is off."""
        c = self.cfg
        st = self.cur.setdefault("base_spring", {"streak": 0, "open": True, "progress": 0.0,
                                                 "log_s": 0.0, "adv": True})
        lo = math.log(float(c.base_spring_min))
        if d_steps > 0:
            step = -lo * d_steps / max(float(c.base_spring_decay_steps), 1.0)
            before = st["log_s"]
            lo_bar = retreat_frac * gate if retreat_frac > 0 else gate
            st["adv"] = ep_len >= (lo_bar if st.get("adv", True) else gate) if gate > 0 else True
            sagging = bool(np.isfinite(defl) and defl > c.base_spring_retreat)
            if sagging:
                st["log_s"] = min(0.0, st["log_s"] + step)
            elif st["adv"]:
                st["log_s"] = max(lo, st["log_s"] - step)
            if before > lo >= st["log_s"]:
                print(f"[ppo] base spring RELEASED at {self.step:,} steps "
                      f"(ep_len {ep_len:.0f}, deflection {defl:.2f})")
        off = st["log_s"] <= lo
        st["progress"] = 1.0 if off else float(st["log_s"] / lo)
        return 0.0 if off else float(math.exp(st["log_s"]))
    ...
    def update_curricula(self, ep_len, d_steps, spring_defl=float("nan")):
        c = self.cfg
        p = self.env_params
        ...
        if getattr(c, "base_spring_enable", False):
            kw["base_spring"] = self._spring_gate(spring_defl, _q("base_spring", d_steps),
                                                  ep_len, gate, rf)
    ...
    def iterate(self):
        ...
        # the base spring's gate signal: mean normalised deflection over the ticks that had one
        sw = m.get("spring_w")
        spring_defl = (float((m["spring_defl"] * sw).sum() / sw.sum())
                       if sw is not None and float(sw.sum()) > 0 else float("nan"))
        log["curriculum/base_spring_defl"] = spring_defl
        # ---- schedules for the NEXT rollout
        self.update_entropy(swing_min, ep_len)
        self.update_curricula(ep_len, d_steps, spring_defl=spring_defl)
        return log
