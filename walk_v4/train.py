"""Train a DASH-01 Walker v2 policy: PPO in JAX over N MJX environments on one GPU.
    ...
    python walk_v4/train.py --preset v2_smoke --steps 4000 --n-envs 8
    # the run (one RTX PRO 6000 / B200; ~4096 envs)
    python walk_v4/train.py --preset v2_s1_planar --steps 300000000 --name v2_s1_planar_s0
    # S1 -> S2 warm start (identical obs/action widths, §11)
    python walk_v4/train.py --preset v2_s2_free --warm-start walk_v4/runs/v2_s1_planar_s0/final.msgpack
    # cluster requeue-safe: the same command is correct for the first start and every restart
    python walk_v4/train.py --preset v2_s2_free --name v2_s2_free_s0 --resume auto

Outputs -> walk_v4/runs/<name>/: ckpt_<steps>.msgpack (+ .json with the curriculum state),
