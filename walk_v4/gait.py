...
ACTION LAYOUT (47; all entries in [-1, 1])
    ...
    [41:47]  residual r0..r5, x residual_scale[j] rad (per joint from v4), per tick

v4: THE REFLEXES ARE GONE FROM THE CONTROL LAW, and since 2026-09-16 so are their three latched
dims -- the action is 47 wide, not 50. They were kept inert for one generation so old bundles still
loaded; nothing loads them now, and a policy that carries log_std for dims which cannot move the
robot is spending entropy on nothing. Measured on the v2 S2 runner (walk_v2/tools/reflex_ablation.py):
the roll reflex's bias held it up and its roll feedback WAS the running gait (21 deg p-p on the
hips, feedback off = upright but -3 m in 20 s); the fixed pitch reflex bought a zero-action policy
nothing (1.07 s vs 1.00 s). Neither had any bandwidth the residual lacks -- same 100 Hz, same IMU,
same delay -- only authority, which is why residual_scale is per joint from here.
...
class GaitParams(NamedTuple):
    ...
    imp_kd_up: float
    imp_kd_dn: float
    residual_scale: tuple        # 6, actuator order: rad per residual unit, PER JOINT (v4)
    freq_lo: float
    freq_hi: float
    ...
    def from_cfg(cls, cfg):
        return cls(cam_amp=float(cfg.cam_amp), thigh_amp=float(cfg.thigh_amp),
                   ...
                   residual_scale=residual_scale6(cfg.residual_scale),
...
def residual_scale6(v):
    """The per-tick authority as a 6-tuple in actuator order. v4 widens the hip and thigh residual
    to carry what the reflexes used to (reflex_ablation: the roll feedback is 21 deg p-p on the
    hips, the pitch reflex reaches 0.25 rad, the v2c residual is +-0.10) while the cams keep the
    narrow one, so this is per joint. A scalar (every config before v4, every resolved_config.json
    on disk) broadcasts, so old runs load, replay and export unchanged."""
    a = np.broadcast_to(np.asarray(v, dtype=np.float64), (6,))
...
def assemble(spec, residual, phi, nominal, p: GaitParams, xp=jnp):
    """Full control law for one tick: (target[...,6], kp[...,6], kd[...,6], q_ref[...,6]).

    target = feedforward(spec, phi) + residual_scale * residual. No reflex term: v4 deleted both
    reflexes and the three latched dims that carried the roll gains (see the module docstring)."""
    q_ref = feedforward(spec, phi, nominal, p, xp)
    target = q_ref + xp.asarray(p.residual_scale) * xp.clip(residual, -1.0, 1.0)
...
def knobs(spec, p: GaitParams, xp=jnp):
    ...
    return xp.clip(spec[..., KNOB_IDX[0]:KNOB_IDX[-1] + 1], -1.0, 1.0)
...
def mirror_action(a, xp=jnp):
    """M on the action: series and impedance profiles unchanged, frequency unchanged, the five
    knobs negated, the residual L<->R and negated."""
    out = a
    for i in KNOB_IDX:
