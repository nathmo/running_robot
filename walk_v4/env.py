...
FRAME_DIM = 34            # v3: + the integrated-gyro heading estimate
PRIV_DIM = 25
TASK_DIM = 2
TWO_PI = 2.0 * np.pi
SPRING_DOFS = ("x", "y", "z", "roll", "pitch", "yaw")


def spring_law(e, ed, s, k0, c0, mask):
    """The 6-DOF base spring's force/torque per DOF: F = -mask (s k0 e + sqrt(s) c0 ed).

    k0 = M w^2 and c0 = 2 M w at full stiffness, so (sqrt(s) c0)^2 = 4 (s k0) M: the spring stays
    CRITICALLY damped at every stiffness the curriculum passes through, rather than ringing more and
    more as it softens (a fixed damper on a softening spring ends up overdamped, a fixed ratio of
    gains ends up underdamped -- neither is the spring that was asked for)."""
    return -mask * (s * k0 * e + jnp.sqrt(s) * c0 * ed)


def spring_anchor(ref, q, rd, dt, leash):
    """Move an anchor at its reference velocity and keep it within `leash` of the body. Beyond the
    leash the body drags the anchor, so on a DOF the actor cannot observe (lateral position) the
    force -- and its bill -- stays bounded instead of growing with every metre of drift."""
    return jnp.clip(ref + rd * dt, q - leash, q + leash)
...
def heading_rate(w, g, euler):
    ...
    v3 estimate. robot/deploy/controller_v2.py computes the same thing in numpy."""
    if not euler:
        return w[2]
...
class EnvParams(NamedTuple):
    ...
    base_spring: float = 0.0     # v4: 6-DOF base spring stiffness, fraction of full (the final point is OFF)
...
class EnvState:
    ...
    spring_mask: jnp.ndarray     # v4: (6,) 1 = this base DOF gets the spring this episode (A2CF masking)
    spring_ref: jnp.ndarray      # v4: (6,) the spring anchors, x y z roll pitch yaw
    roll_lp: jnp.ndarray
...
class DashEnvV2:
    ...
    def __init__(self, cfg, n_envs=None):
        ...
        if getattr(cfg, "base_spring_enable", False):
            # full-stiffness gains per present base DOF, from its natural frequency and the mass it
            # actually presents (plant.spring_M) -- so "2 Hz, critically damped" means that on the body
            pl = self.plant
            w = 2.0 * np.pi * np.asarray(cfg.base_spring_hz, float)[pl.spring_sel]
            self.sp_k0 = pl.spring_M * w ** 2
            self.sp_c0 = 2.0 * pl.spring_M * w
            self.sp_ref = np.asarray(cfg.base_spring_ref, float)[pl.spring_sel]
            self.sp_fref = self.sp_k0 * self.sp_ref
            self.sp_leash = np.asarray(cfg.base_spring_leash, float)[pl.spring_sel]
            print("[env] base spring at full stiffness: " + ", ".join(
                f"{SPRING_DOFS[i]} {w_ / (2 * np.pi):.1f} Hz (M {m_:.2f}, k {k_:.0f}, c {c_:.0f})"
                for i, w_, m_, k_, c_ in zip(pl.spring_sel, w, pl.spring_M, self.sp_k0, self.sp_c0) if w_ > 0))
    ...
    def _reset_one(self, key, params: EnvParams, ov: Override):
        ...
        (k_draw, k_noise, k_pose, k_lib, k_push, k_gust, k_next, k_frame, k_light,
         k_spring) = jax.random.split(key, 10)
        ...
        if c.bringup_enable:
            ...
            data = mjx.forward(mx_i, data)
        # the 6-DOF base spring: which DOFs get it this episode, and where each is anchored -- upright,
        # at stance height, facing the start heading; x and y anchored where the robot starts
        if c.base_spring_enable:
            k_sp = jax.random.fold_in(k_spring, 7)      # its own stream: no existing draw moves
            sp_mask0 = (jax.random.uniform(k_sp, (6,)) >= c.base_spring_mask_p).astype(jnp.float32)
        else:
            sp_mask0 = jnp.ones(6)
        kq = p.key_qpos
        _key = lambda n: jnp.asarray(kq[p.base_q[n]] if p.base_q[n] >= 0 else 0.0, jnp.float32)
        sp_ref0 = jnp.stack([
            data.qpos[p.base_q["x"]].astype(jnp.float32),
            data.qpos[p.base_q["y"]].astype(jnp.float32) if p.base_q["y"] >= 0 else jnp.zeros(()),
            _key("z"), _key("roll"), _key("pitch"), _key("yaw")])
        state = EnvState(
            ...
            spring_mask=sp_mask0, spring_ref=sp_ref0,
            roll_lp=jnp.zeros(()),
    ...
    def _step_one(self, state: EnvState, action, params: EnvParams):
        ...
        # v4: the pitch / roll / yaw training wheels are GONE -- the 6-DOF base spring below replaces
        # all three, and two assist mechanisms in one tree is how a dead code path switches itself on.
        qfrc = jnp.zeros(p.nv)
        # the 6-DOF base spring (walk_v4): the anchors advance once per control tick; the force is
        # computed at every 1 kHz substep below, because a stiff spring held constant for 10 ms is not
        # the critically damped spring it is specified as
        if c.base_spring_enable:
            sp_q = data.qpos[p.spring_qadr]
            sp_rd = jnp.zeros(6).at[0].set(state.v_cmd)[p.spring_sel]     # x's anchor moves at the command
            sp_r = spring_anchor(state.spring_ref[p.spring_sel], sp_q, sp_rd, dt, jnp.asarray(self.sp_leash))
            spring_ref = state.spring_ref.at[p.spring_sel].set(sp_r)
            sp_mask = state.spring_mask[p.spring_sel]
            sp_s = params.base_spring
        else:
            spring_ref = state.spring_ref
        ...
        def substep(carry, k):
            d, tau_sq, con_acc, _, sp_acc = carry
            ...
            if c.base_spring_enable:
                F = spring_law(d.qpos[p.spring_qadr] - sp_r, d.qvel[p.spring_dadr] - sp_rd, sp_s,
                               jnp.asarray(self.sp_k0), jnp.asarray(self.sp_c0), sp_mask)
                d = d.replace(qfrc_applied=qfrc.at[p.spring_dadr].add(F))
                fr = jnp.asarray(self.sp_fref)
                sp_acc = sp_acc + jnp.sum(jnp.where(fr > 0.0, (F / jnp.where(fr > 0.0, fr, 1.0)) ** 2, 0.0))
            d = mjx.step(mx_i, d)
            if c.hold_enable:
            ...
            return (d, tau_sq + tau ** 2, con_acc | con, tau, sp_acc), None

        (data, tau_sq, contact_acc, tau_last, sp_acc), _ = lax.scan(
            substep, (data, jnp.zeros(6), jnp.zeros(2, bool), jnp.zeros(6), jnp.zeros(())),
            jnp.arange(p.decimation))
        # the spring's bill (mean over the substeps) and the gate's signal: how far the ASSISTED DOFs
        # sit from their anchors, in units of base_spring_ref. The gate reads deflection, not force --
        # see ppo._spring_gate for why.
        spring_bill = sp_acc / p.decimation
        if c.base_spring_enable:
            active = (jnp.asarray(self.sp_k0) > 0.0) * sp_mask
            n_act = jnp.sum(active)
            en = (data.qpos[p.spring_qadr] - sp_r) / jnp.asarray(self.sp_ref)
            spring_defl = jnp.sqrt(jnp.sum(active * en ** 2) / jnp.maximum(n_act, 1.0))
            spring_w = (n_act > 0.0).astype(jnp.float32)
        else:
            spring_defl, spring_w = jnp.zeros(()), jnp.zeros(())
        # ---- thermal node
        ...
        rw = self._reward(state, params, data, mx_i, spec, spec_change, residual, motor_cmd, tau_last,
                          ...
                          thermal_x, phi, stop_now, v_target, spring_bill=spring_bill,
        ...
        new_state = state.replace(
            ...
            lp_yaw_true=lp_yaw_true, roll_lp=roll_lp, hist=hist, spring_ref=spring_ref,
            heading_avg=heading_avg,
        ...
        info = dict(
            ...
            spring_defl=spring_defl, spring_w=spring_w,
    ...
    def _reward(self, state, params, data, mx_i, spec, spec_change, residual, motor_cmd, tau,
                grounded, heights, fn, v_body, grav, gyro, lp_yaw_true, crossed, sprint_d,
                thermal_x, phi, stop_now=None, v_target=None, spring_bill=0.0, heading_avg=0.0):
        c, p, gp = self.cfg, self.plant, self.gp
        if stop_now is None:
        ...
        # the 6-DOF base spring's bill: what it had to push, per DOF in units of the force it makes at
        # its reference deflection at full stiffness. NOT shape-ramped: it is the price of the crutch
        # and has to be felt from the first rollout, or nothing asks the policy to stop leaning on it.
        t["base_spring"] = pen(-c.w_base_spring * spring_bill) if c.base_spring_enable else 0.0
        # ---- posture
