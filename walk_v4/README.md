...
```bash
python walk_v4/smoke_test.py                       # adds the v4 block to v3's checks
# stage 1, three seeds per arm (2 V100s each, ~2 h)
for s in 0 1 2; do
  sbatch --gres=gpu:2 --export=ALL,PRESET=v4_stage1,NAME=v4_stage1_s$s,SEED=$s,DEVICES=2,NENVS=4096,NSTEPS=9 walk_v4/slurm/izar_train.sbatch
  sbatch --gres=gpu:2 --export=ALL,PRESET=v4_stage1_reflex,NAME=v4_stage1_reflex_s$s,SEED=$s,DEVICES=2,NENVS=4096,NSTEPS=9 walk_v4/slurm/izar_train.sbatch
done
# stage 2 warm-starts from stage 1's best.msgpack (never final); stage 3 from stage 2's
python walk_v4/tools/compare_runs.py --runs walk_v4/runs/v4_stage1*      # matched steps, greedy ladder
```
...
* **Heading estimator** (`heading_euler`, `env.heading_rate`, mirrored in
  `robot/deploy/controller_v2.py`): the robot's heading integrates the Euler yaw rate
...
Launch: `walk_v4/slurm/izar_v4_one.sh` (three seeds, `PRESET=v4_one_runstop` for the run/stop twin).
...
python walk_v4/train.py --preset v3 --name v3_q_s0 --seed 0 \
       --warm-start walk_v4/runs/v3_stage1_s0/best.msgpack \
       --devices 2 --n-envs 4096 --n-steps 9
# 3  being droppable, and running on a plant that is not the nominal one (~2 h)
python walk_v4/train.py --preset v3_stage3_v24 --name v3_s3_s0 --seed 0 \
       --warm-start walk_v4/runs/v3_q_s0/best.msgpack \
       --devices 2 --n-envs 4096 --n-steps 9
```
...
```bash
python walk_v4/verify.py --run walk_v4/runs/v3_s3_s0
```
...
```bash
pip install -r walk_v4/requirements.txt
python walk_v4/model/make_v2_model.py          # writes model/dash01_v2_*.xml
python walk_v4/tools/make_touch_table.py       # writes model/touch_height.npz (bring-up needs it)
python walk_v4/smoke_test.py                   # ~40 invariants, no GPU, run it before any GPU hour
```
...
WARM=$HOME/running_robot/walk_v4/runs/v3_stage1_s0/best.msgpack \
       walk_v4/slurm/izar_train.sbatch
...
WARM=$HOME/running_robot/walk_v4/runs/v3_q_s0/best.msgpack \
       walk_v4/slurm/izar_train.sbatch
```
...
* **Check the parent THROUGH the warm start, not as it was saved.** `warmstart_var_floor` rewrites
  ...
  command. `python walk_v4/tools/speed_frontier.py --run <parent> --warm-start` loads through the
...
```bash
python walk_v4/export.py --run walk_v4/runs/v3_s0 --out results/v3_s0.npz
python walk_v4/tools/play_joystick.py --bundle walk_v4/results/v3_s0.npz
```
