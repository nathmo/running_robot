"""Compare two v2 traces (walk_v3/tools/trace.py format) from two implementations.

    python walk_v3/tools/compare_traces.py trace_mjx.json trace_cpu.json

Reports:
  1. FUNCTIONAL agreement on shared states: for every tick of trace B, recompute the gait
     targets / kp / kd from B's own (spec, phase, state) with walk_v3/gait.py in numpy and diff
