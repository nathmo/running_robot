"""Train a DASH-01 Walker v2 policy: PPO in JAX over N MJX environments on one GPU.
    ...
    python RLframework/train.py --preset v2_smoke --steps 4000 --n-envs 8
    # the run (one RTX PRO 6000 / B200; ~4096 envs)
    python RLframework/train.py --preset dash --name dash_s0
    # S1 -> S2 warm start (identical obs/action widths, §11)
    python RLframework/train.py --preset dash 
    # cluster requeue-safe: the same command is correct for the first start and every restart
    python RLframework/train.py --preset dash --name dash_s0 --resume auto

Outputs -> RLframework/runs/<name>/: ckpt_<steps>.msgpack (+ .json with the curriculum state),
...
def main():
    ...
    ap.add_argument("--preset", default="dash", choices=sorted(PRESETS))
