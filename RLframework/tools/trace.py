"""Cross-implementation trace: the numbers two implementations of the v2 design must agree on.
    ...
    python RLframework/tools/trace.py --out RLframework/results/trace_mjx.json [--preset dash] [--ticks 300]
    python RLframework/tools/trace.py --rig --out RLframework/results/trace_mjx_rig.json      # welded in the air
    python RLframework/tools/compare_traces.py RLframework/results/trace_mjx.json <other_impl_trace.json>
...
trace's state via RLframework/gait.py in numpy) and the trajectory divergence time.
"""
import argparse
import json
import sys
...
def main():
    ...
    ap.add_argument("--preset", default="dash")
