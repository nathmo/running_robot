"""The §08 margin budget as a batched gate: one greedy policy, every axis, pass/fail per row.
    ...
    python RLframework/tools/eval_envelope.py --run RLframework/runs/dash_s0 [--json out.json]
    python RLframework/tools/eval_envelope.py --run ... --axes push_lat wind_x delay
...
def main():
    ...
    axes = args.axes or ["nominal", "wind_x", "wind_y", "tilt_roll", "tilt_pitch", "friction", "delay",
                         "mass", "thermal", "push_fwd", "push_lat", "lane"]
    if env.plant.planar:
