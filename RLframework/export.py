"""Export a v2 run to a deployment bundle (numpy .npz, no code on load) — bundle version 2.
    ...
    python RLframework/export.py --run RLframework/runs/dash_s0 --out controller/deploy/bundles/dash_s0.npz

The Pi runtime for v2 is `controller/deploy/controller_v2.py` (100 Hz: estimator + actor + the latch +
the free-running clock + gait_v2.py, under the same safety governor as v1). This writes every
number that runtime needs, read off the LIVE env object exactly as controller/deploy/export_policy.py
does for walk_mit -- never re-derived from the config, which is how four separate eval-restore
bugs happened in this project. `controller/deploy/bundle.py` states which meta keys the runtime
...
def export(run, out, checkpoint=None, brake=None, brake_window_s=None):
    ...
    meta = {
        ...
        "frame_dim": int(FRAME_DIM), "history_len": int(cfg.history_len), "history_stride": int(cfg.history_stride),
        ...
        # v3: the heading channel's saturation, so the deploy frame clips where training did
        "heading_cap_rad": float(cfg.heading_cap_rad),
        # v4: the heading integrates the Euler yaw rate (gyro y/z through measured roll/pitch), not gyro z
        "heading_euler": bool(getattr(cfg, "heading_euler", False)),
        ...
        # "command" below for the command channel's own units.
        "v_ceiling": float(cfg.v_ceiling),
        "gait": gait_block,
        "gait_freq_floor_steps": freq_floor_steps,
        "curriculum_source": freq_src,
        "env_params_at_checkpoint": ep,
        # NOT in GaitParams but read by the control law every tick: the commanded-target slew cap
        # and the series order. (The pitch reflex's rate EMA left with the reflexes in v4.)
        "motor_accel_limit": float(cfg.motor_accel_limit),
        "n_harmonics": int(cfg.n_harmonics),
        # v4: per joint (a 6-list); the runtime broadcasts a scalar from older bundles
        "residual_scale": list(gait.residual_scale6(cfg.residual_scale)),
