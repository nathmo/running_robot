# walk_v2 â€” DASH-01 Walker v2 on MJX (GPU-parallel)
...
| 50-dim action, 44 latched at Ï†-wrap + 6 residual | `gait.py` layout; latch in `env._step_one` (`state.commit`) |
| commit flag in the once-block; log-prob mask reads the same flag | obs index `env.wrap_index`; `info["commit"]` â†’ `networks.dim_mask` in the rollout |
| one series per family, Î”/s/o knobs, (+,+) offsets and roll reflex, (+,âˆ’) pitch reflex | `gait.feedforward / reflexes`; sign rule tested in `smoke_test.py` |
| kp(Ï†), kd(Ï†) exp map Ã—2.5/Ã·3, soften-only Ã·4 | `gait.impedance` (a0 is the level, harmonics modulate) |
| contact-triggered resync Îº 0.5 Â± DR, W 0.15 cycle, N_ema 5, 3-cycle warm-up, +1 tick commit | `env._step_one` after physics; a backward pull never uncrosses the wrap |
| obs 33-frame Ã— 10 Ã— stride 2, once-block 44+2(+1), priv tail 25 | `env._frame / _once / _priv` |
| estimator 376â†’128â†’64â†’3 supervised, detached; actor 256Ã—256; critic 401 | `networks.py`, `ppo._est_update` (Adam on the estimator subtree only) |
| symmetry loss w_sym â€–k(M_o s)+k(s)â€–Â² | `networks.ObsMirror` + `ppo.loss_fn`; stats mirror-symmetrized each rollout |
...
| LP yaw (Ï„ 0.7 s) in obs and reward, lane term, one-sided height floor 0.81 m | `lp_yaw_*`, `lane`, `height` |
| thermal Î”T ODE Ï„_th 45 s, penalty > 0.85, hot start U[0,0.7], scale U[0.8,1.2] | `drive.thermal_update`, `plant.draw_plant`; anchors checked in `smoke_test.py` |
| delay 12 ms nominal, U[6,18] ms per episode, substep granular | `drive.live_command` on a 3-tick command buffer |
| armature-fit drive, no EMA target filter | `model/calibrate_plant.py` â†’ `plant_fit.json`, `<motor>` actuators + PD in `drive.py` |
| rigid loop closure + 30 kN/m series spring, 5 mm at 1 BW, Â±50 % DR | see *Plant deviations* |
| DR on: mass/CoM/friction/gains/damping/tilt/zero/IMU + measured IMU noise | `plant.draw_plant`, `env._frame` |
| wind Â±30 N + 30 N gusts, pushes, trips, timing jitter/drop | `env._step_one` |
| two-stage curriculum, frozen interface | presets `v2_s1_planar` (no y/roll/yaw joints) and `v2_s2_free`, same widths |
| gait library pipeline (Â§09) | `gait_lib/` + `spec_source="library"` env variant (action 6+3, once-block 23) |
| Â§08 margin gate | `tools/eval_envelope.py` |
...
* **Series spring location.** The artifact puts the 30 kN/m spring at the pushrod tip. Measured on
  ...
  **5.00 mm vertical at 1 BW on one leg** (23.4 kN/m axial = 29.7 kN/m foot-referred; 10.0 / 3.3 mm
  at the Â±50 % DR corners). The loop closure itself carries the ankle-lock values (solref 0.002).
* **Armature fit is a compromise.** The thigh's in-air corner is set by the link inertia; armature
  can only lower it. Best fit: **6.96 Hz at kp 200/kd 5 (target 6.3, âˆ’0.2 dB peaking) and
...
* **Obs widths 377/402** (policy variant) instead of 376/401: the commit flag is an explicit
  once-block entry (Â§05 rev 2026-09-09), since a resync can move the wrap.
* **Î”_max defaults to 0.6 rad** (the CPG arm's value); `v2_s2_free_wide` is the Â±Ï€ variant (Â§13).
...
* **Constants the artifact leaves open follow the CPU reference's `walk_mit/V2_CONTRACT.md`**
  ...
  0.9 EMA on the rate, w_knob 0.02, pushes U[0.3, 0.6], trips 0.0008, IMU 2Â°, gate 600, the
  residual symmetry term, spec_cycle not billed on the first commit. One deliberate difference
  remains: the phase channel is `(cos Ï†, sin Ï†)` as the artifact's Â§03 table states; the contract
  lists `(sin Ï†, cos Ï†)`.
