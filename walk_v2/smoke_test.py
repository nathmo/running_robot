"""walk_v2 smoke test: the invariants the artifact makes checkable, run before any GPU hour.

    python walk_v2/smoke_test.py            # everything (a few minutes on CPU)
    python walk_v2/smoke_test.py --quick    # no env stepping (seconds)
...
Checks:
  ...
  env      obs layout 377/402 (policy) and 353/378 (library); commit flag at t0; spec latches from
...
def test_env(quick):
    ...
    check("policy variant: actor 377 / obs 402 / action 50", (env.actor_dim, env.obs_dim, env.action_dim) == (377, 402, 50),
          f"{env.actor_dim}/{env.obs_dim}/{env.action_dim}")
    lib = get_config("v2_lib_s2_free")
    lib.n_envs = 4
    envl = DashEnvV2(lib, n_envs=4)
    check("library variant: actor 353 / obs 378 / action 9", (envl.actor_dim, envl.obs_dim, envl.action_dim) == (353, 378, 9),
